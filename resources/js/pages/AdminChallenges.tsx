...
export default function AdminChallenges({ challenges }: AdminChallengesProps) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (editChallenge) {
      router.patch(`/admin/challenges/${editChallenge.id}`, form, {
        preserveScroll: true,
        onFinish: () => { setSubmitting(false); closeForm(); },
      });
    } else {
      router.post('/admin/challenges', form, {
        preserveScroll: true,
        onFinish: () => { setSubmitting(false); closeForm(); },
      });
    }
        ...
        <Card className="bg-white border-none shadow-lg space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                ...
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={() => setForm((f) => ({ ...f, for_all: true }))}
            ...
            <div className="flex gap-3 pt-1">
              <Button type="submit" variant="primary" disabled={submitting || !form.name || !form.start_date || !form.end_date}>
                {submitting ? (
